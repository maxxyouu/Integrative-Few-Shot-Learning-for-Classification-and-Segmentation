...
from model.module.selective_kernel_utils import *
...
class SelectiveKernelAttn(nn.Module):
    """this is the fusion module of the selective kernel convolution"""
    def __init__(self, channels, num_paths=2, attn_channels=32, act_layer=nn.LeakyReLU, norm_layer=None): # nn.BatchNorm2d
        """ Selective Kernel Attention Module

        Selective Kernel attention mechanism factored out into its own module.
        attn_channels is the *d* in the paper
        """
        super(SelectiveKernelAttn, self).__init__()
        self.num_paths = num_paths
        self.fc_reduce = nn.Conv2d(channels, attn_channels, kernel_size=1, bias=False)
        assert norm_layer == None
        self.bn = norm_layer(attn_channels) if norm_layer else None
        self.act = act_layer(inplace=True)
        self.fc_select = nn.Conv2d(attn_channels, channels * num_paths, kernel_size=1, bias=False)

    def forward(self, x):
        _assert(x.shape[1] == self.num_paths, '')
        x = x.sum(1).mean((2, 3), keepdim=True) # add up the path dimention and global average pooling F_gp in the paper
        x = self.fc_reduce(x) # F_fc in the paper
        if self.bn:
            x = self.bn(x)
        x = self.act(x)
        x = self.fc_select(x)
        ...
        x = torch.softmax(x, dim=1) # softmax at the dimention of path.
        return x
...
class SelectiveKernel(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=None, kernel_size=None, stride=1, dilation=1, groups=1,
        ...
        Args:
            ...
            norm_layer (nn.Module): batchnorm/norm layer to use, default as nn.BatchNorm2d
        ...
        conv_kwargs = dict(
            ...
            aa_layer=aa_layer, drop_layer=drop_layer, bias=bias)
